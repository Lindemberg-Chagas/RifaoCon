import { Plus, Edit2, Trash2, LogOut, User, CheckCircle2 } from 'lucide-react';
...
export function Admin() {
  ...
  const handleCreateNew = () => {
    ...
    if (name) {
      ...
      // Desativa as outras e adiciona a nova
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 md:px-8 pt-20 md:pt-28 pb-32 md:pb-16 w-full">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-[#cfa030] hover:bg-[#b58b29] text-[#1e3a8a] px-4 py-2 rounded-full font-bold text-sm shadow-sm transition-colors"
        >
          ...
          <div key={rifa.id} className={`bg-white rounded-2xl p-5 border shadow-sm transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 ${rifa.isActive ? 'border-[#1e3a8a]' : 'border-slate-200'}`}>
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                {rifa.isActive ? (
                  <span className="bg-[#1e3a8a] text-white text-[10px] uppercase font-bold px-2 py-0.5 rounded-full tracking-wider">Ativa</span>
                ) : (
